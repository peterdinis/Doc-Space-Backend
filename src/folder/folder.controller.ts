...
export class FolderController {
  ...
  async createFolder(@Body() createFolderDto: CreateFolderDto & { ownerId: string }) {
    return this.folderService.createFolder(createFolderDto);
  }

  @Get()
  @ApiOperation({ summary: "Get folders with pagination and search" })
  @ApiQuery({ name: "ownerId", required: true, description: "ID of folder owner" })
  @ApiQuery({ name: "search", required: false, description: "Search query for folder name" })
